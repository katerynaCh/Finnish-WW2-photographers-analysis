...
class PascalVocGenerator(Generator):
	def __init__(
		...
		**kwargs
		...
		#changed declaration of self.image_names. We only use people images that are already filtered, so we take the filenames from the filtered folder with people only, instead of the name file.
		for f in os.listdir(os.path.join(data_dir, 'JPEGImages')):
			print(f)
			self.image_names.append(".".join(f.split(".")[:-1]))
		#print(os.listdir(os.path.join(data_dir, 'JPEGImages')))
		#self.image_names = [[".".join(f.split(".")[:-1])] for f in os.listdir(os.path.join(data_dir, 'JPEGImages'))]
